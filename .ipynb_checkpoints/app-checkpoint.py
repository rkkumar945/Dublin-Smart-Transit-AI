...
def fetch_live_dublin_data_from_api():
    ...
    try:
        ...
        # Added per project roadmap (temporal features): these two column
        # names already existed in your CSV from earlier work — this just
        # makes app.py actually generate them (0=Mon..6=Sun, 1-12=month),
        # matching the format your CSV already used.
        df_new["Day_of_Week"] = df_new["Reported_Time"].dt.dayofweek
        df_new["Month"] = df_new["Reported_Time"].dt.month
        return df_new
...
def save_transit_data(df_new: pd.DataFrame, engine) -> None:
    ...
    #
    # Self-healing column alignment: if the CSV already exists (even your
    # existing file with its own column order), new rows are automatically
    # re-ordered/matched to that same header before appending. This avoids
    # ever needing to manually delete or recreate the CSV when a new
    # column is introduced.
    try:
        file_exists = os.path.isfile(CSV_BACKUP_PATH)
        if file_exists:
            existing_columns = pd.read_csv(CSV_BACKUP_PATH, nrows=0).columns.tolist()
            df_to_write = df_new.reindex(columns=existing_columns)
        else:
            df_to_write = df_new
        df_to_write.to_csv(CSV_BACKUP_PATH, mode="a", header=not file_exists, index=False)
...
# Feature set now includes Day_of_Week (0=Mon..6=Sun) alongside Hour, so
# the model can also learn weekday-vs-weekend patterns, not just time of
# day. Older historical rows saved before this change won't have the
# column, so it's safely backfilled from Reported_Time when missing.
if "Day_of_Week" not in df_master.columns:
    df_master["Day_of_Week"] = pd.to_datetime(
        df_master.get("Reported_Time"), utc=True, errors="coerce"
    ).dt.dayofweek.fillna(0)

FEATURE_COLUMNS = ["Hour", "num_docks_available", "Day_of_Week"]
...
def train_production_ml_models(_X, _y_bikes, _y_delay, data_fingerprint: str):
    ...
    if len(_X) < 10:
        ...
        metrics = {"note": "Not enough accumulated data yet for a train/test evaluation."}
        return m_bikes, m_delay, metrics
    ...
    # Per roadmap (Month 3/Phase 6): evaluate with MAE + R2 on a held-out
    # test set instead of just fit-and-forget, and report feature
    # importance so the model isn't a black box.
    bikes_preds = m_bikes.predict(X_test_b)
    delay_preds = m_delay.predict(X_test_d)

    metrics = {
        "note": None,
        "bikes": {
            "mae": round(mean_absolute_error(y_test_b, bikes_preds), 2),
            "r2": round(r2_score(y_test_b, bikes_preds), 3),
        },
        "delay": {
            "mae": round(mean_absolute_error(y_test_d, delay_preds), 2),
            "r2": round(r2_score(y_test_d, delay_preds), 3),
        },
        "feature_importance": dict(
            zip(_X.columns, [round(v, 3) for v in m_bikes.feature_importances_])
        ),
    }
...
model_bikes, model_delay, model_metrics = train_production_ml_models(X, y_bikes, y_delay, fingerprint)
...
if not station_data.empty:
    ...
    current_day = (
        int(station_data["Day_of_Week"].iloc[0])
        if "Day_of_Week" in station_data.columns
        else pd.Timestamp.now().dayofweek
    )
    ...
    live_input = pd.DataFrame([{
        "Hour": current_hour,
        "num_docks_available": total_docks,
    ...
    st.markdown("---")
    with st.expander("📊 Model Performance & Transparency"):
        if model_metrics.get("note"):
            st.caption(model_metrics["note"])
        else:
            bikes_m = model_metrics["bikes"]
            delay_m = model_metrics["delay"]

            mcol1, mcol2 = st.columns(2)
            with mcol1:
                st.markdown("**Bike Availability Model**")
                st.write(f"MAE: **{bikes_m['mae']} bikes**  |  R² Score: **{bikes_m['r2']}**")
                st.caption("Real measured data — these numbers are genuinely meaningful.")
            with mcol2:
                st.markdown("**Transit Delay Model**")
                st.write(f"MAE: **{delay_m['mae']} min**  |  R² Score: **{delay_m['r2']}**")
                st.caption(
                    "⚠️ delay_minutes is a rush-hour rule, not measured real delay — "
                    "this score reflects how well the model learned that rule."
                )

            st.markdown("**Feature Importance (Bike Availability Model)**")
            fi_df = pd.DataFrame(
                sorted(model_metrics["feature_importance"].items(), key=lambda x: -x[1]),
                columns=["Feature", "Importance"],
            )
            st.dataframe(fi_df, use_container_width=True, hide_index=True)

    with st.expander("View Raw Station Record Matrix"):
        display_cols = [c for c in station_data.columns if c != "vehicle_types_available"]
        st.dataframe(station_data[display_cols], use_container_width=True)
else:
